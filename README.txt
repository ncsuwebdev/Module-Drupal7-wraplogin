...
Enable the module in Drupal by going to the Admin Dashboard then clicking the Modules link
    
Set up wraplogin.module by going to the Admin Dashboard then clicking the Configuration link,
then the WRAP login link (under the "PEOPLE" section): 
http://<your-site-url>/overlay=admin/config/people/wraplogin

Enable the WRAP login block by going to the Admin Dashboard then clicking the Structure link, 
and clicking on Blocks: http://<your-site-url>/#overlay=admin/structure/block

Ensure that Clean URLs are enabled for the Drupal instance.
http://<your-site-url>/#overlay=admin/config/search/clean-urls

LOGIN DIRECTORY
---------------
This module will automatically create a directory called "wraplogin" in the root of your 
Drupal installation. Drupal uses this directory and the htaccess file inside it to hook into 
the WRAP module provided by Apache. Do not place any other files in this directory. 
(Unless you need to solve a rewrite problem, see that section below.) 
If you remove this directory and/or the .htaccess inside, they will be recreated when drupal 
cron runs.
